    ...
    if (!list) {
      ...
      switch (list) {
        case "/clients":
          ...
          dispatch({
            type: types.FETCH_CLIENT_ORDERS_BY_STATUS,
            payload: finalRes
          });
          break;
        default:
      ...
      switch (type) {
        ...
        case "/loadings":
          data = res.data.filter(
            item =>
              item.clientID === clientID &&
              item.loadingID.slice(
                3,
                3 + client.data.data.orderLetter.length
              ) === client.data.data.orderLetter
          );
          break;
        default:
        ...
        switch (type) {
          case "/orders":
            arr = data
              .filter(
                d =>
                  d.orderID.slice(0, client.data.data.orderLetter.length) ===
                  client.data.data.orderLetter
              )
              .map(item =>
                item.orderID.slice(client.data.data.orderLetter.length)
              );
            break;
          case "/loadings":
            arr = data.map(item =>
              item.loadingID.slice(client.data.data.orderLetter.length + 3)
            );
            break;
          default:
            return null;
...
export const updateData = (
  ...
  try {
    ...
    if (!stayOnPage) {
      setTimeout(() => {
        history.push(
          `${typeOfData}/${id}${typeOfData === "/clients" ? "/page/1" : ""}`
        );
      }, 1000);
    }
  } catch (err) {
...
// DELETING DATA
export const removeData = (typeOfData, id, data = null) => async dispatch => {
