...
class LoadingCreate extends Component {
  ...
  onChange = async e => {
    ...
    if (e.target.name === "client") {
      ...
      await this.props
        .fetchNewID(client._id, "/loadings")
        .then(response => {
      ...
      await this.props
        .fetchSingleData("/clients", client._id)
        .then(() => {
          const availableOrders = this.props.client.orders.filter(
            order =>
              (order.status === "in" || order.status === "waiting") &
              !order.loadingID
          );
          this.setState({
            ordersList: [...this.state.ordersList, ...availableOrders]
          });
        })
        .catch(error => console.log(error));
    }
          ...
          <div className="ui very padded segment">
            <form onSubmit={this.onSubmit} className="ui form">
              <div className="three fields">
                <div className="field">{this.renderClientSelect()}</div>
                <div className="field">
                  <label htmlFor="loadingID">ID</label>
                  <input
                    type="text"
                    name="loadingID"
                    value={this.state.loadingID}
                    disabled
                  />
                </div>
                <div className="field">
              ...
              <div className="field">
                ...
                <input
                  type="text"
                  name="commentsOnLoading"
                  value={this.state.commentsOnLoading}
                  onChange={this.onChange}
                />
              </div>
